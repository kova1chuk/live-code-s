...
export default function CheckAnagramTestRunner({
  testCases,
  code,
  onRunTests,
}: CheckAnagramTestRunnerProps) {
  ...
  const runTests = () => {
    ...
    try {
      ...
      const isAnagram = new Function(code + "\nreturn isAnagram;")();

      const testResults = testCases.map((testCase) => {
        try {
          const [str1, str2] = testCase.input.split(",").map((s) => s.trim());
          const actual = isAnagram(str1, str2);
          const passed = actual === (testCase.expected === "true");
