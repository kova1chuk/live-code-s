...
interface MoveZeroesTestRunnerProps {
  testCases: TestCase[];
  code: string;
  onRunTests: () => void;
}

export default function MoveZeroesTestRunner({
  testCases,
  code,
  onRunTests,
}: MoveZeroesTestRunnerProps) {
  ...
  const runTests = () => {
    ...
    try {
      ...
      const moveZeroes = new Function(code + "\nreturn moveZeroes;")();

      const testResults = testCases.map((testCase) => {
        try {
          const input = [...testCase.input]; // Create a copy to avoid modifying original
          const actual = moveZeroes([...input]); // Create another copy for the function
          const passed =
